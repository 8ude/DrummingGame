...
namespace FluffyUnderware.DevToolsEditor
{
    ...
    public class DTInspectorNode : IComparable
    {
        ...
        {
            var res = Attributes.FindAll(x => x.GetType().IsAssignableFrom(typeof(T)));
            var r = new T[res.Count];
        ...
        public bool FindNode<T>(string name, out T node) where T : DTInspectorNode
        {
            ...
            {
                foreach (var it in Items)
                    if (it.FindNode<T>(name, out node))
                        return true;
            }
            return false;
        }
        ...
        public override void Calculate(bool firstRun=false)
        {
            ...
            foreach (IDTFieldRenderAttribute a in Attributes)
            {
                ...
                else if (serializedProperty != null)
                {
                    ...
                    {
                        var condA = (FieldConditionAttribute)a;
    ...
    public class DTGroupNode : DTInspectorNode
    {
            ...
            foreach (IDTGroupRenderAttribute a in Attributes)
            {
               if (a is GroupAttribute)
                ...
                else if (serializedObject != null)
                {
                    ...
                    {
                        var condA = (GroupConditionAttribute)a;
        ...
        {
            DTGroupNode node;
            if (FindNodeAt<DTGroupNode>(nameAndPath,out node))
    ...
    public class DTPersistentState
    {
        ...
        public static GUIStyle TabbarButton
        {
            get
            {
                if (mTabbarButton == null)
                {
                    ...
                    mTabbarButton.padding.top = 4;
                    mTabbarButton.padding.bottom = 4;
                    mTabbarButton.margin = new RectOffset(0, 0, 0, 0);
                }
