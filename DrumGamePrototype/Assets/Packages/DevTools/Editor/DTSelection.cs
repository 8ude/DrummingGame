...
namespace FluffyUnderware.DevToolsEditor
{
    public static class DTSelection
    {
        ...
        {
            if (Selection.gameObjects.Length==1 || (Selection.gameObjects.Length>1 && allowMultiSelection))
        ...
        public static Vector3 GetPosition()
        {
            ...
            var tt = DTSelection.GetAllAs<Transform>();
            if (tt.Count > 0)
            {
        ...
        {
            var S = Selection.GetFiltered(typeof(T), SelectionMode.TopLevel | SelectionMode.ExcludePrefab | SelectionMode.Editable);
            var L = new List<T>();
        ...
        {
            var gl = new List<GameObject>(objects.Length);
            foreach (Component c in objects)
                if (c != null && c.gameObject != null && !gl.Contains(c.gameObject) && !Selection.Contains(c))
                    gl.Add(c.gameObject);
            if (gl.Count > 0)
            {
                var cur = Selection.gameObjects;
                Selection.objects=cur.AddRange<GameObject>(gl.ToArray());
            }
        }
        ...
        {
            if (!MuteEvents && Selection.activeGameObject != mCurrentGameObject || mCurrentGameObjectsCount!=Selection.gameObjects.Length)
            {
                mCurrentGameObject = Selection.activeGameObject;
                mCurrentGameObjectsCount = Selection.gameObjects.Length;
                raiseOnSelectionChange();
